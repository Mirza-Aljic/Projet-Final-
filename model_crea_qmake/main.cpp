...
int main(int argc, char *argv[])
{
    ...
    progressWindow.setMaximum(EPOCHS);

    QObject::connect(button1, &QPushButton::clicked, [button2, button4, &manager, HIDDEN_SIZE, INPUT_SIZE, OUTPUT_SIZE, LEARNING_RATE, EPOCHS, &progressWindow](){
        manager.deleteAllEntriesHO();
        manager.deleteAllEntriesIH();
        button2->setEnabled(true);
        button4->setEnabled(true);
        QVector<QVector<double>> dataMatrix = manager.fetchTables();
        QVector<QVector<double>> featuresMatrix;
        QVector<double> tagsMatrix;
        progressWindow.show(); // Affiche la fenêtre

        for (const QVector<double>& row : dataMatrix) {
            if (row.isEmpty())
                continue;

            QVector<double> features = row;
            double tag = features.takeLast(); // Retire la dernière valeur (le tag) et la retourne

            featuresMatrix.append(features);
            tagsMatrix.append(tag);
        }

        // Affichage des features
        for (int i = 0; i < featuresMatrix.size(); ++i) {
            QStringList rowStrings;
            for (double val : featuresMatrix[i]) {
                rowStrings << QString::number(val);
            }
            qDebug() << "Feature Row" << i << ":" << rowStrings.join(", ");
        }

        // Affichage des tags
        for (int i = 0; i < tagsMatrix.size(); ++i) {
            qDebug() << "Tag Row" << i << ":" << QString::number(tagsMatrix[i]);
        }
            ...
            for(int i = 0; i < featuresMatrix.size(); i++) {
                ...
                for(int j = 0; j < HIDDEN_SIZE; j++) {
                    ...
                    for(int k = 0; k < INPUT_SIZE; k++) {
                        sum += featuresMatrix[i][k] * weights_ih[j][k];
                    }
                ...
                // Calcul de l'erreur
                double error = tagsMatrix[i] - output;
                ...
                for(int j = 0; j < HIDDEN_SIZE; j++) {
                    ...
                    for(int k = 0; k < INPUT_SIZE; k++) {
                        weights_ih[j][k] += LEARNING_RATE * d_hidden * featuresMatrix[i][k];
                    }
                }
        ...
        for(int i = 0; i < HIDDEN_SIZE; i++) {
            for(int j = 0; j < INPUT_SIZE; j++) {
                ...
                float weight = static_cast<float>(GLOBALweights_ih[i][j]);
                manager.sendDataIH(j, i, weight);
                QThread::msleep(10);
            }
        }
        ...
        for(int i = 0; i < OUTPUT_SIZE; i++) {
            for(int j = 0; j < HIDDEN_SIZE; j++) {
                ...
                float weight = static_cast<float>(GLOBALweights_ho[i][j]);
                manager.sendDataHO(i, j, weight);
                QThread::msleep(10);
            }
        }
